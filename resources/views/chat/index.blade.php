        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('chat.new') }}"
                   class="inline-flex items-center px-4 py-2 bg-[#B91C1C] text-white text-sm rounded-lg hover:opacity-90 transition shadow-md">
                    ✨ Chat Baru
                </a>
            </div>
        </div>
                ...
                <div id="chat-messages" class="flex-1 overflow-y-auto p-6 space-y-4">
                    ...
                    @if($histories->isEmpty())
                        <div class="flex flex-col items-center justify-center h-full text-center">
                            ...
                            <h3 class="text-xl font-bold text-gray-700 dark:text-gray-200 mb-2 uppercase tracking-tight">Halo, {{ Auth::user()->name }}!</h3>
                            <p class="text-gray-500 dark:text-gray-400 max-w-md font-medium">
                                Saya <strong class="text-accent italic">Sinau AI</strong>, asisten belajar pintarmu. Tanyakan apa saja — matematika, sains, bahasa, sejarah, atau topik apapun!
                            </p>
                            <div class="mt-6 grid grid-cols-2 gap-3 w-full max-w-lg">
                                @foreach([
                                    '📐 Jelaskan teorema Pythagoras',
                                    '🧪 Apa itu fotosintesis?',
                                    '📝 Bantu saya buat esai',
                                    '🔢 Cara selesaikan persamaan'
                                ] as $suggestion)
                                <button onclick="sendSuggestion(this)"
                                        data-text="{{ $suggestion }}"
                                        class="suggestion-btn text-left p-3 bg-accent/5 dark:bg-gray-700 hover:bg-accent/10 dark:hover:bg-gray-600 rounded-xl text-sm text-accent dark:text-accent/80 transition border border-accent/10 dark:border-gray-600">
                                    {{ $suggestion }}
                                </button>
                                @endforeach
                            </div>
                        ...
                        @foreach($histories as $chat)
                            @if($chat->role === 'user')
                                <div class="flex justify-end">
                                    <div style="background-color: #B91C1C;" class="max-w-[75%] text-white rounded-2xl rounded-tr-sm px-4 py-3 shadow-md">
                                        <p class="text-sm leading-relaxed font-semibold">{{ $chat->message }}</p>
                                        <span class="text-xs text-white/60 mt-1 block text-right font-black">{{ $chat->created_at->format('H:i') }}</span>
                                    </div>
                                </div>
                            @else
                                <div class="flex justify-start gap-3">
                                    <div class="flex-shrink-0 w-10 h-10 bg-white rounded-xl flex items-center justify-center border border-gray-100 shadow-sm overflow-hidden">
                                    ...
                                    <div class="max-w-[75%] bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-100 rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm border border-gray-50 dark:border-gray-600">
                                        <div class="text-sm leading-relaxed prose prose-sm dark:prose-invert max-w-none ai-response font-medium">{{ $chat->message }}</div>
                                        <span class="text-xs text-gray-400 mt-1 block uppercase tracking-tighter">{{ $chat->created_at->format('H:i') }}</span>
                                    </div>
                                </div>
                            @endif
                        @endforeach
                    ...
                    <form id="chat-form" class="flex gap-3 items-end">
                        ...
                        <div class="flex-1 relative">
                            <textarea
                                ...
                                class="w-full resize-none rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-100 px-4 py-3 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-accent/50 transition shadow-inner font-medium"
                                style="max-height: 120px;"
                            ></textarea>
                        </div>
                        <button type="submit" id="send-btn"
                                style="background-color: #B91C1C;" 
                                class="flex-shrink-0 w-12 h-12 hover:opacity-95 text-white rounded-xl flex items-center justify-center transition shadow-lg disabled:opacity-50 disabled:cursor-not-allowed border-b-4 border-[#991B1B]">
                ...
                chatMessages.insertAdjacentHTML('beforeend', `
                    <div class="flex justify-end">
                        <div style="background-color: #B91C1C;" class="max-w-[75%] text-white rounded-2xl rounded-tr-sm px-4 py-3 shadow-md">
                            ...
                            <span class="text-xs text-white/60 mt-1 block text-right font-black">${now}</span>
                        </div>
                    </div>
                `);
            } else {
